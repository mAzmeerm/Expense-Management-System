...
if (!$row) {
    ...
    if ($row['Status'] == "Pending" || $row['Status'] == "pending") {
        ...
        if (isset($_POST['approve'])) {
            ...
            // Fetch remaining budget (FIXED: Changed c.Date to c.ClaimDate based on your ERD)
            ...
            // Update budget and approve if requested balance is sufficient
            ...
            //Updated status query (Removed quotes around $claimID)
            $sqlstatus = "UPDATE ExpenseClaim SET Status='Rejected' WHERE ClaimID = $claimID";
            mysqli_query($dbconn, $sqlstatus) or die("Error updating claim status: " . mysqli_error($dbconn));
