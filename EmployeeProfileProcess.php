<?php
session_start();
include("dbconn.php");
include("function.php");
require_login();
$loggedInUser = $_SESSION['UserID'];

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    // Only escape and grab fields the user is actually allowed to edit
    $name = mysqli_real_escape_string($dbconn, $_POST['name']);
    $phone = mysqli_real_escape_string($dbconn, $_POST['phone']);
    $password = $_POST['password'];
    
    $sqlUpdate = "UPDATE employee 
                  SET Name = '$name', 
                      PhoneNum = '$phone'";


    if (!empty($password)) {
        $sqlUpdate .= ", Password = '" . mysqli_real_escape_string($dbconn, hash_password($password)) . "'";
    }
    
    $sqlUpdate .= " WHERE EmployeeID = '$loggedInUser'";
    
    if (mysqli_query($dbconn, $sqlUpdate)) {
        set_alert('success', '<span class="menu-item-wrapper"><img src="IconSuccess.svg" alt="Success" width="20" height="20" style="margin-right: 5px;"> Profile updated successfully.</span>', 'EmployeeProfile.php');
    } else {
        set_alert('error', 'Error updating profile: ' . mysqli_error($dbconn), 'EmployeeProfile.php');
    }
} else {
    header("Location: EmployeeProfile.php");
    exit();
}
?>